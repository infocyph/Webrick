...
2. Development and production runtimes are distinct. Development may discover and reflect. Production consumes prevalidated artifacts.
3. InterMix 10 is the DI runtime. Update Webrick from `^9.2` to `^10.0` and use its compiled/production capabilities.
4. DI is conditional, not fundamental. A static zero-argument closure must never enter InterMix.
5. Request construction is conditional. Routing must be possible without materializing the complete Webrick Request.
6. Request scope is conditional. Do not establish an InterMix request scope unless the compiled execution plan proves it is required.
7. Static/worker state is immutable after boot. No route, middleware, request, trace, transport or user state may live in mutable process-global/static state.
8. Swoole/OpenSwoole, RoadRunner and Workerman are first-class persistent runtimes, but never dependencies of the core.
9. FPM/Apache remains first-class. Persistent-worker optimization must not turn Webrick into a Swoole-only framework.
10. HTTP correctness is part of performance. Incorrect behavior at higher RPM is a regression.
...
# 5. Production boot from one prevalidated artifact

Webrick 5 should emit one versioned production bundle containing all required runtime information:
...
Provide a production boot API equivalent to:

`RouterKernel::fromCompiledArtifact(...)`

The registrar-driven boot path becomes development/build oriented.

When strict production mode is enabled and the artifact is absent/stale, fail boot. Never silently fall back to scanning/reflection in production.

---

# 6. Integrate InterMix 10 properly

Update dependency:

`infocyph/intermix: ^10.0`

Use InterMix 10's production container and compiled invocation path only where DI is actually needed.
...
Use InterMix 10 production invocation only when dependency injection is required.

## Scope rule

Open a request scope only when:
...
Use InterMix 10 `ClosureSerializer` as the single closure serialization abstraction.
...
`Response` must not reach into the global InterMix container to render views.

Move view rendering to a dedicated responder/helper/service using `ViewFactoryInterface`.
...
- load InterMix 10 production container
...
Production prevalidated mode should avoid expensive repeated validation.

Artifact format/version mismatch should fail boot quickly.

---

# 55. Atomic artifact publication

Extend existing atomic route-cache publication guarantees to the entire compiled runtime artifact.

A worker/request must observe either:

- the old complete artifact
...
Use InterMix 10 ClosureSerializer.
...
- `Dispatcher.php` — major rewrite around ExecutionPlan.
...
- `RouterKernel.php` — major compiled runtime rewrite; no unconditional Request/Invoker/scope.
...
- `RouteCache.php` — compiler/artifact publisher.
...
Especially test Swoole with concurrently suspended/resumed coroutines.
...
- controller with compiled InterMix DI
- one middleware
- five middleware
- scoped service
...
Integrate InterMix 10 production runtime.
...
materialize required request portions
        ↓
open request scope only when required
        ↓
compiled middleware
        ↓
InterMix 10 compiled invocation only when required
        ↓
Response
        ↓
...
- InterMix 10 production DI is integrated
...
- persistent-worker soak tests pass
...
> More work happens once before traffic; far less work happens per request.
