...
**📖 Note**: This guide focuses on how Response helpers work **with** content negotiation middleware. For comprehensive Response API reference (all helpers, status codes, headers, etc.), see `Response Helpers <./responses.rst>`__.

--------------

This guide shows how ``Response`` helpers and the negotiation middleware work together. The examples reflect the real behavior in the codebase.
...
``HttpFactory`` creates Webrick message objects with familiar factory method names. It does not implement PSR-17 and the returned response does not implement the PSR-7 response interface:
...
When another framework owns the response, copy the Webrick status, headers, protocol and body into that framework's response through an explicit adapter. Do not pass a Webrick response to a parameter type-hinted as ``Psr\Http\Message\ResponseInterface``.
