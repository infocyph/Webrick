...
This document closes the source-level deep correctness revision defined in ``docs/webrick-5-deep-correctness-revision.rst`` on branch ``webrick-5/batch-1-correctness``.
...
- ☒
  ...
  3. Response-cache scheme isolation and cache-key versioning

- ☒

  4. Signed URL ignored-parameter generation/verification symmetry

- ☒

  5. Signed URL canonical decimal integer grammar

- ☒

  6. Removal of request-derived media-extension static memoization

- ☒

  7. Shared structured JSON/XML media-type classification

- ☒

  8. Generic added-header duplicate preservation

- ☒

  9. ``withUri(..., preserveHost: true)`` Host port preservation

- ☒

  10. Request application-input isolation from cookie/server/env compatibility variables

- ☒

  11. Malformed uploaded-file normalization

- ☒

  12. Complete stream initialization writes

- ☒

  13. Strict request Content-Length and charset parsing
...
- ☒
  ...
  15. Strict shared HTTP-date parsing

- ☒

  16. UAParser Android/Client-Hint/Edge corrections
...
- ☒
  ...
  18. Case-insensitive response-helper header handling

- ☒

  19. Exact cookie Max-Age semantics
...
- ☒
  ...
  21. Public Range helper unification with canonical range parsing
...
- ☒
  ...
  23. Case-insensitive authoritative range response headers

- ☒

  24. CLI emitter HEAD/bodyless/streaming parity

- ☒

  25. Runtime file fast paths use captured ``FileBody`` metadata

- ☒

  26. CORS Private Network Access ``Vary`` semantics

- ☒

  27. Bounded/validated telemetry request IDs

- ☒

  28. PSR producer-backed response conversion
...
- ☒
  ...
  30. PSR uploaded-file preservation or explicit unsupported failure
...
- ☒
  ...
  32. Compressed IPv6 route constraints

- ☒

  33. Paired PCRE delimiter support
...
- ☒
  ...
  36. Language helper qvalue/empty-list/stable-order behavior

- ☒

  37. RateLimit non-negative/invariant guards

- ☒

  38. Vary field-name token validation

- ☒

  39. HTTP grammar parser consolidation

- ☒

  40. Repository response-header casing reconciliation

- ☒

  41. Strict numeric/configuration reconciliation

- ☒

  42. Persistent-worker static-state reconciliation
