...
final class EndUser
{
    ...
    public static function setTrustedProxies(array $cidrs): void { Request::setTrustedProxies($cidrs); }
    ...
    {
        $peer=$this->ipNoProxy();
        if($peer===null||!$this->isTrustedProxy($peer)){return $peer;}
        return $this->ipViaProxy();
    }

    public function ipNoProxy(): ?string
    {
        if($this->cachedNoProxy!==null){return $this->cachedNoProxy;}
    ...
    {
        if($this->cachedViaProxy!==null){return $this->cachedViaProxy;}
        $peer=$this->ipNoProxy();
        if($peer===null||!$this->isTrustedProxy($peer)){return $this->cachedViaProxy=$peer;}
        $chain=$this->forwardedChain();
        if($chain===[]){$vendor=$this->explicitVendorClientIp();if($vendor!==null){$chain[]=$vendor;}}
        $candidate=$peer;
        for($i=count($chain)-1;$i>=0;$i--){if(!$this->isTrustedProxy($candidate)){break;}$candidate=$chain[$i];}
    ...
    public function parseUserAgent(): array
    {
        ...
        foreach($parsed as $key=>$value){$out[$key]=$value;}
        return $out;
    }
    public function userAgent(): ?string { return $this->req->getHeaderLine('User-Agent')?:null; }
    ...
    private static function maskedIp(string $ip): ?string
    {
        ...
        if(!is_string($mask)){return null;}
        $masked=inet_ntop($bin&$mask);return is_string($masked)?$masked:null;
    }
    ...
    private static function normalizeIpToken(string $ip): array
    {
        ...
        $zonePos=strpos($plainIp,'%');if($zonePos!==false){$plainIp=substr($plainIp,0,$zonePos);}
        return [$plainIp,$wrapped];
    }

    private function explicitVendorClientIp(): ?string
    {
        foreach($this->trustedClientIpHeaders as $header){$value=trim($this->req->getHeaderLine($header));if($value===''){continue;}$first=trim(explode(',',$value,2)[0]);$validated=filter_var($first,FILTER_VALIDATE_IP);if(is_string($validated)){return $validated;}}
        return null;
    }
    ...
    {
        $chain=$this->parseForwarded();if($chain!==[]){return $chain;}
        if(($this->proxyHeaderFlags()&Request::HEADER_X_FORWARDED_FOR)===0){return [];}
        $line=$this->req->getHeaderLine('X-Forwarded-For');
        if($line===''){$serverLine=$this->req->getServerParams()['HTTP_X_FORWARDED_FOR']??null;$line=is_string($serverLine)?$serverLine:'';}
    ...
    {
        if(($this->proxyHeaderFlags()&Request::HEADER_FORWARDED)===0){return [];}
        $header=$this->req->getHeaderLine('Forwarded');
        if($header===''){$serverHeader=$this->req->getServerParams()['HTTP_FORWARDED']??null;$header=is_string($serverHeader)?$serverHeader:'';}
        if($header===''||preg_match_all('/(?:^|[,;]\s*)for=(?:"?\[?)([A-F0-9:.]+)(?:\]?"?)/i',$header,$matches)===false){return [];}
        return $this->validIpList($matches[1]);
    }

    private function proxyHeaderFlags(): int { return $this->forwardedHeaderMask??Request::getProxyHeaderFlags(); }
    /** @param array<int,string> $values @return list<string> */
    private function validIpList(array $values): array
    {
        $out=[];foreach($values as $value){[$candidate]=self::normalizeIpToken(trim($value));$validated=filter_var($candidate,FILTER_VALIDATE_IP);if(is_string($validated)){$out[]=$validated;}}
        return $out;
    }
}
