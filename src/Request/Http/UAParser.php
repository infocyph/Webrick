...
final class UAParser
{
    ...
    private function browser(): array
    {
        ...
        if ($brands) {
            ...
            foreach ($preferred as $brand => $ver) {
                if (!preg_match('/^(?:Chromium|Not\s?)?A?Brand$/i', (string) $brand)) {
        ...
        foreach (self::$tokenMap as $token => $label) {
            if (str_contains($this->uaLower, (string) $token)) {
    ...
    private function platform(): string
    {
        ...
        if ($this->hint) {
            ...
            if ($this->hint['mobile'] && !str_contains(strtolower((string) $plat), 'android')) {
                $plat .= ' Mobile';
            }
