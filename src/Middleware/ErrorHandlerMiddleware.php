 ...
 *
 * • Uses Status enum for reason phrases & semantics
 * • Negotiates response: JSON / XML / HTML / text via Accept (or negotiated.type attr)
 * • Optional PHP-error→exception conversion
 * • Adds safe headers + Vary: Accept
 * • Logs via PSR-3
 *
 * Place early in the pre-route stack.
 */
final class ErrorHandlerMiddleware
{
    ...
    private function render(Request $req, Throwable $e, int $status): Response
    {
        ...
        $public = $reason; // default display message
        $msg = $this->debug ? ($e->getMessage() ?: $public) : $public;

        // Body suppressed for empty statuses (safety)
    ...
    {
        // Minimal well-formed XML; escape basics
        ...
        $exEl = $this->debug ? '<exception>' . $xe($e::class) . '</exception><file>' . $xe(
            $e->getFile(),
        ) . ':' . $e->getLine() . '</file>' : '';
        return <<<XML
